...
use App\Factories\AimlTagFactory;
...
class AimlParser
{
    ...
    public function startElement($parser, $name, array $attributes)
    {
        ...
        //if this is a conidition tag then set some extra bits if the condition is right
    ...
    public function cdata($parser, $cdata)
    {
        ...
        if ($this->currentTag->getIsTagValid()) {
            if ($this->currentTag->getTagName()=='Template') {
               ...
               // echo "<br/>THIS IS JUST PROCCESS CONTENTS...".htmlentities($cdata)." parent = ".$p->getTagName();
                $this->currentTag->processContents($cdata);
            }
        } else {
            LemurLog::warn(
    ...
    public function endElement($parser, $name)
    {
        ...
        if ($this->currentTag->getIsTagValid()) {
            ...
            if ($currentResponse!='') {
                $setResponse=true;
                $this->setResponse($currentResponse, 'final');
            }
        } else {
            $currentResponse='';
