    ...
    public function score($categories)
    {
        ...
        foreach ($categories as $category) {
            ...
            if ($category->topic==='') {
                ...
                $category->topic === $topicNormalised || preg_match('/^'.$category->regexp_topic."$/i", $topicNormalised, $matches) )) {
                $points = 30;
                $scoreArray[$i]['total'] = $this->addScoreToCategory($points, $scoreArray[$i]);
            ...
            } elseif ( $category->that!=='' && (
                    $category->that === $thatNormalised || preg_match('/^'.$category->regexp_that."$/i", $thatNormalised, $matches) )) {
                $points = 30;
                $scoreArray[$i]['total'] = $this->addScoreToCategory($points, $scoreArray[$i]);
        ...
        //we have big problems there should always be atleast 1
