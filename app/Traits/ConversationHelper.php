...
trait ConversationHelper
{
    ...
    {
        if ($default=='') {
            $default = config('lemur_tag.unknown.var_property');
        }
    ...
    {
        return $this->hasOne(\App\Models\Turn::class, 'conversation_id')->latest('turns.id');
    }
    ...
    public function getGlobalProperty($name, $default = '')
    {
        ...
        if ($property===null) {
            if ($default=='') {
                $default = config('lemur_tag.unknown.global_property');
            }
