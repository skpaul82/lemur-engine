...
class BotPolicyAdminTest extends TestCase
{
    private $user;
    private $policy;
    private $bot;
    ...
    public function setUp() :void
    {
        ...
        $this->bot = factory(Bot::class)->create(['user_id'=>$user->id, 'language_id'=>1]);
    }
    ...
    {
        $response = $this->policy->view($this->user, $this->bot);
        $this->assertTrue($response->allowed());
    }
    ...
    {
        $response = $this->policy->update($this->user, $this->bot);
        $this->assertTrue($response->allowed());
    }

    public function testCanDeleteAsAdmin()
    {
        $response = $this->policy->delete($this->user, $this->bot);
        $this->assertTrue($response->allowed());
    }

    public function testCanRestoreAsAdmin()
    {
        $response = $this->policy->restore($this->user, $this->bot);
        $this->assertTrue($response->allowed());
    }

    public function testCanForceDeleteAsAdmin()
    {
        $response = $this->policy->forceDelete($this->user, $this->bot);
        $this->assertTrue($response->allowed());
    }
